...
function App() {
  ...
  const dailySession = useMemo(() => {
    ...
    return [
      retrieval ? { id: 'retrieval', tag: 'RECALL', title: retrieval.title, detail: retrieval.detail, action: 'Review due card', page: retrieval.kind === 'opening' ? 'openings' : 'practice', target: retrieval.kind === 'opening' ? 'study' : retrieval.kind === 'endgame' ? 'endgame-route' : 'game-lab', area: retrieval.kind === 'opening' ? 'opening' : retrieval.kind === 'endgame' ? 'end' : phase, retrieval } : { id: 'repertoire', tag: 'OPENING', title: savedOpenings[0] ? openings.find((item) => item.id === savedOpenings[0])?.name || 'Your repertoire' : 'Choose one opening', detail: 'Train one line until you can state its plan and defensive idea.', action: 'Open repertoire', page: 'openings', target: 'study', area: 'opening' },

...
function App() {
  ...
  return <main className={`app workspace-${page} profile-${profile.status} ${theme === 'dark' ? 'dark' : ''}`}>
    ...
    {page === 'practice' && <section className="practice-flight" aria-label="Practice routes"><div><p className="eyebrow">PRACTICE FLIGHT DECK</p><h2>Choose the<br /><em>right work.</em></h2><p>Different chess problems need different training. Pick the mode that matches the mistake you want to stop making today.</p></div><div className="practice-routes"><button onClick={() => document.querySelector('#puzzle-zone')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}><span>01 · CALCULATE</span><strong>Daily tactics</strong><small>Forcing moves, tactical discipline</small></button><button onClick={() => document.querySelector('#game-lab')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}><span>02 · PLAN</span><strong>Middlegame lab</strong><small>Find the break and improve the worst piece</small></button><button onClick={() => document.querySelector('#structure-atlas')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}><span>03 · STRUCTURE</span><strong>Pawn atlas</strong><small>Read the long-term plan from the pawns</small></button><button onClick={() => document.querySelector('#endgame-route')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}><span>04 · CONVERT</span><strong>Endgame route</strong><small>Exact technique and tablebase truth</small></button></div></section>}

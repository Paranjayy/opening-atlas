...
function App() {
  ...
  return <main className={`app workspace-${page} ${openingRouteMode} ${practiceRouteMode} profile-${profile.status} ${theme === 'dark' ? 'dark' : ''}`}>
    ...
    {page === 'review' && review && <aside className="review-move-rail" aria-label="Game move sheet"><div className="review-rail-heading"><span>GAME SHEET</span><strong>{review.headers.White || 'White'} · {review.headers.Black || 'Black'}</strong><small>{review.headers.Result || '*'} · {reviewPly} / {review.moves.length} plies</small></div><div className="review-rail-moves">{review.moves.map((move, index) => <button className={reviewPly === index + 1 ? 'current' : ''} onClick={() => setReviewPly(index + 1)} key={`${index}-${move.san}`}><span>{`${Math.ceil((index + 1) / 2)}${index % 2 === 0 ? '.' : '…'}`}</span><strong>{move.san}</strong><i>{move.captured ? '×' : move.flags.includes('k') || move.flags.includes('q') ? '♜' : move.piece === 'p' ? '•' : ''}</i></button>)}</div><div className="review-rail-actions"><button onClick={() => setReviewPly(0)}>Start position ↞</button><button onClick={() => setReviewPly(review.moves.length)}>Final position ↠</button><button onClick={analyseReviewPosition}>Analyse current position ↑</button><p><kbd>←</kbd><kbd>→</kbd> or <kbd>H</kbd><kbd>L</kbd> moves · <kbd>W</kbd><kbd>S</kbd> ends</p></div></aside>}
